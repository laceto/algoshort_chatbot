...
def make_generate_node(llm: ChatOpenAI):
    async def generate(state: State) -> State:
        ...
        async for chunk in llm.astream(messages):
            ...
            # Optionally, you can yield or send partial output here if integrated with UI
...
st.title("LangGraph RAG Chatbot with Streaming")
...
if prompt := st.chat_input("Ask a question..."):
    ...
    with st.chat_message("assistant"):
        ...
        # Stream the graph output incrementally
        response_chunks = []

        async def stream_graph():
            # Use graph.stream() with streamMode="messages" to get LLM tokens
            async for msg_chunk, metadata in graph.stream(
                {"question": prompt, "context": "", "answer": ""},
                stream_mode="messages"
            ):
                # Filter tokens from the generate node only
                if metadata.langgraph_node == "generate" and msg_chunk.content:
